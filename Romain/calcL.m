function [L,Lderiv] = calcL(lambda, n, a, b, c, signm, signn)
...
for i=1:size(V,2)
    ...
    psi(:,i) = lambda.^(1-n+2*r).*signCKT1.*sqrt(abs(lambda.^2-h^2));
    psideriv(:,i) = lambda.^(-n+2*r).*signCKT1.*sqrt(abs(lambda.^2-h^2)) ...
	.* ((1-n+2*r)+(lambda.^2./(abs(lambda.^2-h^2))));
    for j=1:size(V,1)
