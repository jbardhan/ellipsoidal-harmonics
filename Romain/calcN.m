function [N,Nderiv] = calcN(lambda, n, a, b, c, signm, signn)
h = sqrt(a^2-b^2);
k = sqrt(a^2-c^2);
...
for i=1:size(V,2)
    ...
    psi(:,i) = lambda.^(n-2*r).*signCKT1.*signCKT2.*sqrt(abs((lambda.^2-k^2).*(lambda.^2-h^2)));
    psideriv(:,i)=lambda.^(n-2*r-1).*signCKT1.*signCKT2.*sqrt(abs((lambda.^2-k^2).*(lambda.^2-h^2))).*((n-2*r)+(2*lambda.^4-(k^2+h^2)*lambda.^2)./((lambda.^2-k^2).*(lambda.^2-h^2)));
    for j=1:size(V,1)
